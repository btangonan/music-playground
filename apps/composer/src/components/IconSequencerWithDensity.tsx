...
export default function IconSequencerWithDensity(props: IconSequencerWithDensityProps) {
  ...
  // NEW: octave shift state and derived pitch bounds (C3..B5 by default)
  const [octaveOffset, setOctaveOffset] = useState(0); // clamp UI to [-3, +3]
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const divisor = SNAP_DIVISOR[resolution];
    const absoluteRawBar = x / SIXTEENTH_WIDTH;
    const snappedBar = Math.round(absoluteRawBar / divisor) * divisor;
    const maxValidBar = 63;
    const finalSnappedBar = Math.max(0, Math.min(maxValidBar, snappedBar));
    ...
    if (DEBUG) {
      console.log('🎯 DROP:', {
        resolution, divisor, mouseX: x.toFixed(1), mouseY: y.toFixed(1), col, row, absoluteRawBar: absoluteRawBar.toFixed(2), snappedBar, finalSnappedBar, pitch, snapCenterX: (finalSnappedBar * SIXTEENTH_WIDTH + SIXTEENTH_WIDTH / 2).toFixed(1) + 'px'
      });
    }
  ...
  const renderGrid = () => {
    ...
    for (let row = 0; row < TOTAL_SEMITONES; row++) {
      ...
      for (let col = 0; col < TIME_STEPS; col++) {
        ...
        if (resolution === '1/4') { subdivisionLines.push(0); }
        else if (resolution === '1/8') { subdivisionLines.push(0, COLUMN_WIDTH / 2); }
        else if (resolution === '1/16') { subdivisionLines.push(0, SIXTEENTH_WIDTH, COLUMN_WIDTH / 2, SIXTEENTH_WIDTH * 3); }
        cells.push(
  ...
  const renderPlacements = () => {
    return placements.map((p, index) => {
      ...
      if (DEBUG) {
        console.log(`📍 ICON ${index}:`, { bar: p.bar, resolution, targetLeft: targetLeft.toFixed(1) + 'px', row, rowCenter: rowCenter.toFixed(1) + 'px', pitch: p.pitch });
      }
  ...
  return (
    <div>
      {/* Octave shift controls (no layout changes) */}
